...
from cellpilot.data.module import BaseDataModule
from cellpilot.bo.optimizer import BotorchOptimizer


from cellpilot.metrics import (
...
from cellpilot.utils.config import flatten

from jsonargparse import (
    ArgumentParser,
    ActionConfigFile,
)
from cellpilot.utils.config import instantiate_class
from botorch.acquisition import AcquisitionFunction
from cellpilot.surrogate_models.gp import SurrogateModel
...
def configure_pooling_method(config, model_name):
    ...
    if (
        ...
        in ["cellpilot.surrogate_models.gp.DeepGP"]
    ):
        config["surrogate_model"]["init_args"]["finetuning_model"]["init_args"][
            "pooling_method"
        ] = hugging_face_models[model_name]
...
def validate_configuration(config):
    ...
    if surrogate_class == "cellpilot.surrogate_models.gp.GP" and representation == "get_tokens":
...
def train(config):
    ...
    with wandb.init(
        project="CellPilot", config=wandb_config, group=config["group"]
    ) as run:
